...
public partial class Index : IDisposable
{
    ...
    {
        SetMaxSpeed(_config.UserMaxSpeed + _config.IncrementAmount);
    }
    ...
    private async Task RequestDevice()
    {
        ...
        if (_device != null)
        {
            ...
            {
                _treadmillService =
                    await _device.Gatt.GetPrimaryService(BluetoothServices.FTMS); // Fitness Machine Service
                _treadmillDataCharacteristic =
                    await SubscribeToNotifications(BluetoothCharacteristic.TreadmillData, OnTreadmillDataChanged);
                _treadmillControlPoint =
                    await SubscribeToNotifications(BluetoothCharacteristic.FitnessMachineControlPoint,
                        OnControlPointChanged);
                _treadmillStatusCharacteristic =
                    await SubscribeToNotifications(BluetoothCharacteristic.FitnessMachineStatus,
                        OnTreadmillStatusChanged);
            }
            catch (Exception ex)
            {
                Console.WriteLine(ex.Message);
    ...
    private async Task<IBluetoothRemoteGATTCharacteristic?> SubscribeToNotifications(string characteristicUuid,
    ...
    {
        await _treadmillControlPoint.Start();
    }

    private async Task Stop()
    {
        await _treadmillControlPoint.Stop();
    }

    private async Task Pause()
    {
        await _treadmillControlPoint.Pause();
    }
    ...
    private async Task IncreaseSpeed()
    {
        ...
        {
            await _treadmillControlPoint.SetTargetSpeed(_liveData.Speed + _config.IncrementAmount,
                _config.UserMaxSpeed);
        }
        else
        {
            var s = _liveData.Speed + _config.IncrementAmount;
            if (s <= _config.UserMaxSpeed) _liveData.Speed = s;
        }
    }
    ...
    private async Task DecreaseSpeed()
    {
        ...
        {
            var s = _liveData.Speed - _config.IncrementAmount;
            if (s >= _config.EquipmentMinSpeed) _liveData.Speed = s;
        }
    }

    private async Task SendReset()
    {
        await _treadmillControlPoint.Reset();
    }
    ...
    private void HandleStopOrPause(byte[] value)
    {
        ...
        Console.WriteLine(controlStatus == 0x02
            ? "Treadmill is paused."
            : "Treadmill is stopped."); // Assuming 0x01 means paused
    }


    private void OnTreadmillDataChanged(object? sender, CharacteristicEventArgs args)
    {
        HandleNotifications(args.Value);
    }
    ...
    private void OnControlPointChanged(object? sender, CharacteristicEventArgs args)
    {
        HandleFtmsControlPoint(args.Value);
    }

    private void HandleFtmsControlPoint(byte[] value)
    {
        var opcode = value[0];
        var result = value.Length > 1 ? value[1] : (byte)0;
