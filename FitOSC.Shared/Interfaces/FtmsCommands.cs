...
public static class FtmsCommands
{
    ...
    public static async Task ExecuteCommand(this IBluetoothRemoteGATTCharacteristic? characteristic, byte opcode, params byte[] values)
    {
...
public static FtmsData ReadFtmsData(byte[] value)
{
    ...
    int posStepCount = -1; // New variable for step count
    ...
    if ((flags & (1 << 13)) != 0) { posStepCount = nextPosition; nextPosition += 2; } // Step count flag
    ...
    if (posKcal != -1)
    {
        treadmillData.Calories = BitConverter.ToUInt16(value, posKcal);
    }
    ...
    if (posStepCount != -1)
    {
        treadmillData.Steps = BitConverter.ToUInt16(value, posStepCount);
    }
